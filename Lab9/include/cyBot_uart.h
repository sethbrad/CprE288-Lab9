...
// Initialize the UART to communication between CyBot and PuTTy (Note: only
// allowed for testing purposes)
void cyBot_uart_init(void);

// Clean CyBot to PuTTy UART initialization, before running your UART GPIO init
// code
void cyBot_uart_init_clean(void);

// First half (GPIO) of PuTTY to CyBot UART initialization (Note: only allowed
...
// Send a byte over the UART from CyBot and PuTTy (Baud Rate 115200, No Parity,
// No Flow Control)
void cyBot_sendByte(char data);

// Cybot waits (i.e. blocks) to receive a byte from PuTTy (Buad Rate 115200, No
// Parity, No Flow Control) returns byte that was received
char cyBot_getByte_blocking(void);

// Nonblocking call to receive over uart1/
//  Check if a Byte was received from PuTTy, if so place in location of
//  byte-sized variable passed to the function
