...
void ping_init (void)
{
    ...
    //Sets the prescaler to to extend to 24 bits (With maximum value since we are counting down)
    TIMER3_TBPR_R |= 0xFF;

    //Sets the 16-bit timer with a maximum loaded value (Because we are counting down)
...
//    GPIO_PORTB_PCTL_R |= 0x7000;
    ...
    //Re-enable Alternate function direction should automatically be set back to an input
    GPIO_PORTB_AFSEL_R |= 0x8;
