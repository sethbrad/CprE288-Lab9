 ...
 *  @edit: Diane Rover 02/01/20: Corrected comments about ordering of switches
 * for new LCD board and added busy-wait on PRGPIO
 */
...
#warning "Unimplemented function: void button_init()"
  ...
  // start, e.g., execute a simple dummy assignment statement, such as "long
  // delay = SYSCTL_RCGCGPIO_R".
  // Instead, use the PRGPIO register and busy-wait on the peripheral ready bit
  // for PORTE. while ((SYSCTL_PRGPIO_R & 0x??) == 0) {}; 2) Set the buttons as
  // inputs, do not modify other PORTE wires GPIO_PORTE_DIR_R &=
 ...
 * @return the position of the rightmost button being pushed. 1 is the leftmost
 * button, 4 is the rightmost button.  0 indicates no button being pressed
 */
...
#warning "Unimplemented function: uint8_t button_getButton(void)"	// delete warning after implementing
  ...
  // ----------------------------------------
  // In embedded programming, often we only care about one or a few bits in a
  // piece of data.  There are several bitwise operators that we can apply to
  // data in order to "mask" the bits that we don't care about.
  //
  //	| = bitwise OR		& = bitwise AND		^ = bitwise XOR
  //~ = bitwise NOT
  //		  << x = shift left by x bits		 >> x = shift right by x
  //bits
  //
  // Let's say we want to know if push button 3 (Sw3) is pushed.  Sw3 is bit 2
  // of GPIO_PORTE_DATA_R. Since push buttons are high (1) initially, and low
  // (0) if pushed, PORTE should look like: GPIO_PORTE_DATA_R => 0b???? ?0?? if
  // Sw3 is pushed GPIO_PORTE_DATA_R => 0b???? ?1?? if Sw3 is not pushed
  //
